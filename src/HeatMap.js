...
const HeatmapTable = ({ students, removeStudent }) => {
  ...
  useEffect(() => {
    ...
    const cellWidth = 60; // Width for rectangular cells
    const cellHeight = 40; // Height for rectangular cells
    ...
    // Add skill labels (rows) on the left side
    ...
    pivotData.forEach((d, rowIndex) => {
      students.forEach((student, colIndex) => {
        ...
        const cellGroup = g.append('g') // Group for each cell to manage events and texts
        ...
        cellGroup.append('rect')
          .attr('x', 0)
          .attr('y', 0)
          .attr('width', cellWidth - 10) // Subtracting 10 for spacing
        ...
        cellGroup.append('text')
          ...
          .attr('y', (cellHeight - 10) / 2)
          .attr('dy', '.35em')
          .attr('text-anchor', 'middle')
          .text(() => {
            if (isSpecial) {
              return d.skill === 'Experience' ? `${score} yrs` : '';
            } else {
              return `${score}`;
            }
          })
  ...
  if (students.length === 0) {
    return (
      <section className='w-full h-full flex justify-center items-center text-lg font-semibold text-gray-500'>
  ...
  return (
    <section ref={containerRef} className="relative overflow-auto">
      <svg ref={svgRef}></svg>
      {tooltip.visible && (
        <div
          ref={tooltipRef}
